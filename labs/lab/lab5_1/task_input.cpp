/**
 * @file task_input.cpp
 * @brief Lab 5.1 keypad input task implementation.
 */
...
static void enterManualMode(Lab5ControlState *state) {
...
void vTaskLab5Input(void *pvParameters) {
    (void)pvParameters;
    ...
    for (;;) {
        ...
        if (key != 0) {
            ...
            switch (key) {
                ...
                case 'D':
                    state->hysteresisBandC += HYSTERESIS_STEP_C;
                    if (state->hysteresisBandC > HYSTERESIS_MAX_C) {
                        state->hysteresisBandC = HYSTERESIS_MIN_C;
                    }
                    printf("[INPUT] Hysteresis band changed\r\n");
                    break;
