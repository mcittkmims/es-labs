...
The system follows a layered architecture that organizes functionality into distinct levels, each with specific responsibilities. This structure separates concerns and enables modular development. The architecture consists of the following layers:

\begin{itemize}
    \item \textbf{APP (Application Layer)} --- implements command processing logic and high-level functionalities.
    \item \textbf{SRV (Service Layer)} --- provides common services such as command parsing and data formatting.
    \item \textbf{ECAL (ECU Abstraction Layer)} --- abstracts platform-specific hardware details, including STDIO-to-UART redirection.
    \item \textbf{MCAL (Microcontroller Abstraction Layer)} --- interfaces with microcontroller peripherals (LED GPIO control).
    \item \textbf{HW (Hardware)} --- represents the physical ATmega2560 microcontroller and its hardware peripherals.
\end{itemize}
