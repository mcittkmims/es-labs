...
\subsubsection{Test: Command Persistence}

An important observation emerges when repeatedly issuing the same command. Sending \texttt{led on} while the LED is already illuminated maintains its on state without any visible change:

\begin{lstlisting}[caption=Serial output --- repeated LED ON command, label=lst:test_led_on_repeat]
> led on
[OK] LED is now ON.
> led on
[OK] LED is now ON.
...
Similarly, sending \texttt{led off} when the LED is already off still maintains the off state. This behavior is expected and demonstrates the unidirectional nature of the LED control interface. The microcontroller sends control signals to the LED but does not receive feedback about its current state. The system operates in open-loop control mode---commands are executed unconditionally without verifying the LED's previous state.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{resources/ObtainedResults/test_led_on_repeat.png}
\caption{Repeated \texttt{led on} command maintains LED state}
\label{fig:test_led_on_repeat}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{resources/ObtainedResults/test_unknown_command.png}
\caption{Error handling for unrecognized commands}
\label{fig:test_unknown_command}
\end{figure}
