...
\begin{tikzpicture}[node distance=0.45cm]
    ...
    % Place sublayer nodes at fixed positions inside the task layer
    \node[sublayer, fill=green!20, minimum width=2.8cm] at ($(task.center)+(-3.2cm,0)$) (t1) {Task 1};
    \node[sublayer, fill=green!20, minimum width=2.8cm] at (task.center)               (t2) {Task 2};
    \node[sublayer, fill=green!20, minimum width=2.8cm] at ($(task.center)+(3.2cm,0)$) (t3) {Task 3};
...
\begin{tikzpicture}
    ...
    % Fixed-coordinate layout: PC left, MCU centre, LEDs stacked right, Button below MCU
    \node[dev] (pc)   at (0,   0)    {PC Terminal};
    \node[dev] (mcu)  at (5,   0)    {ATmega2560\\{\scriptsize TaskScheduler}};
    \node[hw]  (btn)  at (5,  -2.2)  {Push Button\\{\scriptsize Pin 7, INPUT\_PULLUP}};
    \node[hw]  (gLed) at (10,  1.5)  {Green LED\\{\scriptsize Pin 8, OUTPUT}};
    \node[hw]  (rLed) at (10,  0)    {Red LED\\{\scriptsize Pin 9, OUTPUT}};
    \node[hw]  (yLed) at (10, -1.5)  {Yellow LED\\{\scriptsize Pin 10, OUTPUT}};

    % Connections
    \draw[bidirarrow] (pc)  -- node[above, font=\scriptsize]{USB / UART0} (mcu);
    \draw[outarrow, dashed] (mcu) -- node[right, font=\scriptsize]{Read (active-LOW)} (btn);
...
\centering
\begin{tikzpicture}
    % Explicit coordinates — diamonds need ~2.5cm centre-to-centre clearance
    \node (start) [startstop] at (0,   0.0)  {schedulerRun()};
    \node (init)  [process]   at (0,  -1.5)  {now = millis()\\chosen = $-1$, earliest = MAX};
    \node (dec1)  [decision]  at (0,  -3.8)  {i $<$ count?};
    \node (dec2)  [decision]  at (0,  -6.5)  {tasks[i] due\\\&\& earliest?};
    \node (upd)   [process]   at (5.5,-6.5)  {chosen = i\\earliest = nextRun[i]};
    \node (inc)   [process]   at (0,  -9.0)  {i++};
    \node (dec3)  [decision]  at (0, -11.3)  {chosen $\geq$ 0?};
    \node (run)   [process]   at (0, -13.5)  {tasks[chosen].funcPtr()\\nextRun += period};
    \node (stop)  [startstop] at (0, -15.0)  {return};

    \draw [arrow] (start) -- (init);
    \draw [arrow] (init)  -- (dec1);
    \draw [arrow] (dec1)  -- node[anchor=east]{\scriptsize Yes} (dec2);
    \draw [arrow] (dec1.west) -- ++(-3.5,0) |- node[above, near start]{\scriptsize No} (dec3.west);
    \draw [arrow] (dec2)  -- node[above]{\scriptsize Yes} (upd);
    \draw [arrow] (dec2)  -- node[anchor=east]{\scriptsize No}  (inc);
    \draw [arrow] (upd.east)  -- ++(0.8,0) |- (inc.east);
    % Back-edge routed LEFT to avoid passing through dec2
    \draw [arrow] (inc.west)  -- ++(-2.0,0) |- (dec1.west);
    \draw [arrow] (dec3)  -- node[anchor=east]{\scriptsize Yes} (run);
    \draw [arrow] (dec3.east) -- ++(2.0,0) |- node[above, near start]{\scriptsize No} (stop.east);
    \draw [arrow] (run)   -- (stop);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}
    % Explicit coordinates — avoids trapezoid/process overlap from relative positioning
    ...
    \node (dec1)  [decision]  at (0,  -6.2)  {Press completed?};
    \node (led)   [process]   at (5.5, -6.2) {Light GREEN or RED LED\\set auto-off timer};
    \node (dec2)  [decision]  at (0,  -8.7)  {Green timer\\expired?};
    \node (goff)  [process]   at (5.5, -8.7) {GREEN LED off\\timer = 0};
    \node (dec3)  [decision]  at (0, -11.2)  {Red timer\\expired?};
    \node (roff)  [process]   at (5.5,-11.2) {RED LED off\\timer = 0};
    \node (stop)  [startstop] at (0, -13.5)  {Task 1 End};

    \draw [arrow] (start) -- (read);
    \draw [arrow] (read)  -- (fsm);
    \draw [arrow] (fsm)   -- (dec1);
    \draw [arrow] (dec1)  -- node[above]{\scriptsize Yes} (led);
    \draw [arrow] (led.west)  -- (2.5, -6.2) -- (2.5, -8.7) -- (dec2);
    \draw [arrow] (dec1)  -- node[anchor=east]{\scriptsize No}  (dec2);
    \draw [arrow] (dec2)  -- node[above]{\scriptsize Yes} (goff);
    \draw [arrow] (goff.west) -- (3.0, -8.7) -- (3.0, -11.2) -- (dec3);
    \draw [arrow] (dec2)  -- node[anchor=east]{\scriptsize No}  (dec3);
    \draw [arrow] (dec3)  -- node[above]{\scriptsize Yes} (roff);
    \draw [arrow] (roff.west) -- (2.5, -11.2) -- (2.5, -13.5) -- (stop);
...
\centering
\begin{tikzpicture}
    % Explicit coordinates — right column kept far enough right and each row
    % has sufficient vertical clearance so stats/blink/clr never overlap tog.
    \node (start) [startstop] at (0,   0.0)  {Task 2 Start};
    \node (dec1)  [decision]  at (0,  -2.2)  {g\_newPress?};
    \node (stats) [process]   at (6.5, -2.2) {Update counters\\g\_totalPresses++\\g\_shortPresses or g\_longPresses++};
    \node (blink) [process]   at (6.5, -4.2) {Arm blink: steps = 10 (short)\\or 20 (long), LED ON};
    \node (clr)   [process]   at (6.5, -6.0) {g\_newPress = false};
    \node (dec2)  [decision]  at (0,  -8.0)  {steps $>$ 0\\AND 100\,ms elapsed?};
    \node (tog)   [process]   at (6.5, -8.0) {Toggle yellow LED\\steps-{}-\\reset timer};
    \node (dec3)  [decision]  at (0, -10.5)  {steps == 0?};
    \node (off)   [process]   at (6.5,-10.5) {Yellow LED off};
    \node (stop)  [startstop] at (0, -12.5)  {Task 2 End};

    \draw [arrow] (start) -- (dec1);
    \draw [arrow] (dec1)  -- node[above]{\scriptsize Yes} (stats);
    \draw [arrow] (stats) -- (blink);
    \draw [arrow] (blink) -- (clr);
    \draw [arrow] (clr.west)  -- (3.0, -6.0) -- (3.0, -8.0) -- (dec2);
    \draw [arrow] (dec1)  -- node[anchor=east]{\scriptsize No}  (dec2);
    \draw [arrow] (dec2)  -- node[above]{\scriptsize Yes} (tog);
    \draw [arrow] (tog.west)  -- (3.5, -8.0) -- (3.5, -10.5) -- (dec3);
    \draw [arrow] (dec2)  -- node[anchor=east]{\scriptsize No}  (dec3);
    \draw [arrow] (dec3)  -- node[above]{\scriptsize Yes} (off);
    \draw [arrow] (off.west)  -- (3.0, -10.5) -- (3.0, -12.5) -- (stop);
    \draw [arrow] (dec3)  -- node[anchor=east]{\scriptsize No}  (stop);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}
    % Explicit y coordinates — extra gap around the io (trapezoid) node
    % to prevent it extending into the process nodes above and below.
    \node (start) [startstop] at (0,  0.0)  {Task 3 Start (10\,s)};
    \node (snap)  [process]   at (0, -1.8)  {Snapshot statistics:\\total, shorts, longs, totalMs};
    \node (avg)   [process]   at (0, -3.6)  {avgMs = total $>$ 0 ? totalMs / total : 0};
    \node (print) [io]        at (0, -5.6)  {printf() formatted\\10s report to STDIO};
    \node (reset) [process]   at (0, -7.6)  {Reset all accumulators\\to zero};
    \node (stop)  [startstop] at (0, -9.4)  {Task 3 End};
