...
\subsection{Serial Input Module (SerialInput.h / SerialInput.cpp)}

\begin{lstlisting}[language=C++, caption={Serial line reader with echo --- encapsulates all raw Serial calls}]
// SerialInput.h
class SerialInput {
public:
    SerialInput(uint8_t maxLen = 31);
    void init();
    // Returns complete line or nullptr if not ready yet
    const char* readLine();
    bool available() const;
private:
    static const uint8_t MAX_BUF_SIZE = 64;
    char    _buf[MAX_BUF_SIZE];
    uint8_t _maxLen, _idx;
};

// SerialInput.cpp - all low-level Serial I/O lives here
